# Decisions

This directory stores short architecture decision records for the repository knowledge base.

## Format

Each ADR should use this structure:

- Title
- Status
- Date
- Context
- Decision
- Consequences

## Status Values

- accepted
- superseded
- deprecated

## Current ADRs

- `core/custom/decisions/0001-docs-live-in-core-custom.md`
- `core/custom/decisions/0002-template-registry-is-standard.md`
- `core/custom/decisions/0003-define-php-for-project-secrets.md`
- `core/custom/decisions/0004-manager-config-files-must-deploy-via-git.md`
- `core/custom/decisions/0005-main-controller-namespace-and-base-controller-are-required.md`
- `core/custom/decisions/0006-local-live-examples-are-preferred-over-internet-search.md`
- `core/custom/decisions/0007-views-and-external-frontend-assets-are-one-implementation-unit.md`
- `core/custom/decisions/0008-components-should-be-installed-through-project-packages.md`
