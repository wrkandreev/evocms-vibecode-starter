...
- Creating TVs through migrations is deferred. When registry write API supports the exact `bLang` field, prefer it so metadata, template links, and real TVs stay synchronized. Use the verified CMS workflow only when write API support is unavailable.
...
4. Seed standard localized core fields when the endpoint is supported:
...
- `POST /api/template-registry/blang/default-params`
  - seeds standard `blang_tmplvars` and synchronizes generated localized TVs
- `POST/PATCH/DELETE /api/template-registry/blang/fields`
